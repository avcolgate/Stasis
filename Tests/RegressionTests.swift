...
struct RegressionTests {
    static func main() {
        ...
        check(BatteryReading.isCharging(liveCurrent: 2.95, externalConnected: true), "live current into the battery means charging")
        check(!BatteryReading.isCharging(liveCurrent: 0, externalConnected: true), "idle battery at the limit is not charging")
        check(!BatteryReading.isCharging(liveCurrent: -0.6, externalConnected: true), "discharging while attached is not charging")
        check(!BatteryReading.isCharging(liveCurrent: 2.95, externalConnected: false), "without external power the battery is not charging")
        var currentFilter = StaleCurrentFilter()
        check(currentFilter.filter(currentMilliamps: -441, connected: false, now: 0) == -441, "current passes through without a plug event")
        check(currentFilter.filter(currentMilliamps: -441, connected: true, now: 1) == nil, "discharge current is stale right after plugging in")
        check(currentFilter.filter(currentMilliamps: -441, connected: true, now: 60) == nil, "current stays stale until IOKit refreshes it")
        check(currentFilter.filter(currentMilliamps: 0, connected: true, now: 61) == 0, "refreshed current is trusted again")
        check(currentFilter.filter(currentMilliamps: 0, connected: true, now: 62) == 0, "trusted current stays trusted")
        check(currentFilter.filter(currentMilliamps: 0, connected: false, now: 70) == nil, "idle current is stale right after unplugging")
        check(currentFilter.filter(currentMilliamps: 0, connected: false, now: 70 + StaleCurrentFilter.maximumStaleness) == 0, "unchanged current is trusted after the staleness limit")
        var stalePlugged = BatteryMetrics()
        stalePlugged.externalConnected = true
        check(ChargingMode.current(battery: stalePlugged) == .pluggedIn, "unknown current while plugged in is not shown as discharge")
